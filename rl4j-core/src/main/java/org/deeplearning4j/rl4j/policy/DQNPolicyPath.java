 ...
 */
@AllArgsConstructor
public class DQNPolicyPath<O extends Encodable> extends Policy<O, Integer> {

	final private IDQN dqn;

	public static <O extends Encodable> DQNPolicyPath<O> load(String path) throws IOException {
		return new DQNPolicyPath<O>(DQN.load(path));
	}
	...
	public <AS extends ActionSpace<Integer>> double play(MDP<O, Integer, AS> mdp, IHistoryProcessor hp) {
		...
		while (!mdp.isDone()) {
			...
			if (step % skipFrame != 0) {
				...
				// 选择节点
				if (mdp instanceof PathEnv) {
					PathEnv pathEnv = (PathEnv) mdp;
