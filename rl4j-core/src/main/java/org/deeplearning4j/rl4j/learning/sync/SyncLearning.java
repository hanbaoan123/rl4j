...
/**
 * Mother class and useful factorisations for all training methods that are not
 * asynchronous.
 *
...
public abstract class SyncLearning<O extends Encodable, A, AS extends ActionSpace<A>, NN extends NeuralNet>
	 ...
	 * - the number of steps reaches the maximum defined in the configuration
	 * (see {@link LConfiguration#getMaxStep() LConfiguration.getMaxStep()})<br>
	 * OR<br>
	 * - a listener explicitly stops it<br>
	 * <p>
	 * Listeners<br>
	 * For a given event, the listeners are called sequentially in same the
	 * order as they were added. If one listener returns
	 * {@link TrainingListener.ListenerResponse
	 * SyncTrainingListener.ListenerResponse.STOP}, the remaining listeners in
	 * the list won't be called.<br>
	 * Events:
	 * <ul>
	 * <li>{@link TrainingListener#onTrainingStart() onTrainingStart()} is
	 * called once when the training starts.</li>
	 * <li>{@link TrainingListener#onNewEpoch(IEpochTrainer) onNewEpoch()} and
	 * {@link TrainingListener#onEpochTrainingResult(IEpochTrainer, IDataManager.StatEntry)
	 * onEpochTrainingResult()} are called for every epoch.
	 * onEpochTrainingResult will not be called if onNewEpoch stops the training
	 * </li>
	 * <li>{@link TrainingListener#onTrainingProgress(ILearning)
	 * onTrainingProgress()} is called after onEpochTrainingResult()</li>
	 * <li>{@link TrainingListener#onTrainingEnd() onTrainingEnd()} is always
	 * called at the end of the training, even if the training was cancelled by
	 * a listener.</li>
	 * </ul>
	 */
	public void train() {
	...
	protected abstract IDataManager.StatEntry trainEpoch(); // TODO: finish
															// removal of
															// IDataManager from
															// Learning
}
